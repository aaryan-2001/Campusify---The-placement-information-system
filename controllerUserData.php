...
if(isset($_POST['signup'])){
	...
	$year = mysqli_real_escape_string($con, $_POST['year_of_graduation']);
	$class = mysqli_real_escape_string($con, $_POST['class']);
	$fn = $_FILES['student_college_id']['name'];
	$tmp = $_FILES['student_college_id']['tmp_name'];
	$uploads_dir='C:/xampp/htdocs/campusify/student_details';
    ...
    elseif (strlen($year) != 4) {  
        echo"<script>alert('Year must have 4 digits.')</script>";  
    }
    elseif (!preg_match ("/^[0-9]*$/", $year) ){ 
        ...
        $status = "notverified";
        $insert_data = "INSERT INTO usertable (name, email, password, code, status, mobile, dept, year, student_college_id, class)
                        values('$name', '$email', '$encpass', '$code', '$status', '$mobile', '$dept', '$year', '$fn', '$class')";
    ...
    if(isset($_POST['change-password'])){
        ...
        if($password !== $cpassword){
            ...
            if($run_query){
                ...
                $_SESSION['info'] = $info;
				echo "<script>alert('Your Password has Changed Successfully');window.location='login-user.php'</script>";
